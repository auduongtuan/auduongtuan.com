...
function Dialog({ children, open, onClose, title, className }: DialogProps) {
  ...
  return (
    ...
    >
      <BaseDialog.Portal>
        <BaseDialog.Backdrop className="bg-modal-backdrop fixed inset-0 z-(--stack-modal) opacity-100 transition-opacity duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0" />
        <BaseDialog.Popup
          className={cn(
        ...
        >
          <div className="shadow-elevation-high border-control-outline flex w-full max-w-md translate-z-0 flex-col overflow-hidden rounded-xl border border-solid">
            <header className="bg-card relative z-1 flex items-center justify-between rounded-t-[11px] px-3 py-1.5">
              ...
              <BaseDialog.Title className="text-primary w-full px-8 py-0.5 text-center text-sm font-semibold">
                {title}
              </BaseDialog.Title>
            </header>
